...
async def test_connection_stability():
    ...
    for i in range(5):
        ...
        try:
            ...
            logger.info(
                f"✅ Attempt {i + 1}: {len(tools)} tools loaded in {connection_time:.2f}s"
            )
...
async def main():
    ...
    logger.info(
        f"📊 Success Rate: {success_rate:.1%} ({len(successful_attempts)}/{len(stability_results)} attempts)"
    )

    if successful_attempts:
        avg_time = sum(r["connection_time"] for r in successful_attempts) / len(
            successful_attempts
        )
